...
ON nexus_tenant_owner_membership (
    owner_id,
    status
);

COMMENT ON TABLE nexus_tenant IS
    'NEXUS tenant security boundary. Only ACTIVE tenants may use protected APIs.';

COMMENT ON TABLE nexus_owner_identity IS
    'NEXUS durable owner identity status boundary.';

COMMENT ON TABLE nexus_tenant_owner_membership IS
