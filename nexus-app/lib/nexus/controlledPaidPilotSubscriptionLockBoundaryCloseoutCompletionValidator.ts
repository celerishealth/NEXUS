...
type CloseoutCompletionValidatorStatus = "pass" | "fail";

type CloseoutCompletionValidatorCheck = {
  id: string;
  label: string;
  status: CloseoutCompletionValidatorStatus;
  evidence: string;
};

export type ControlledPaidPilotSubscriptionLockBoundaryCloseoutCompletionValidator = {
  completionValidatorId: "controlled-paid-pilot-subscription-lock-boundary-closeout-completion-validator-v1";
  title: "NEXUS Controlled Paid Pilot Subscription Lock Boundary Closeout Completion Validator v1";
  mode: "read-only-preview-closeout-completion-validator";
  day: 244;
  overallStatus: CloseoutCompletionValidatorStatus;
  validatedCompletionSummaryId: "controlled-paid-pilot-subscription-lock-boundary-closeout-completion-summary-v1";
  checks: CloseoutCompletionValidatorCheck[];
...
function pass(
  ...
  label: string,
  evidence: string
): CloseoutCompletionValidatorCheck {
  return {
    id,
    label,
    status: "pass",
...
export function validateControlledPaidPilotSubscriptionLockBoundaryCloseoutCompletionSummary(): ControlledPaidPilotSubscriptionLockBoundaryCloseoutCompletionValidator {
  const summary =
  ...
  const checks: CloseoutCompletionValidatorCheck[] = [
    summary.mode === "read-only-preview-closeout-completion-summary"
      ? pass(
          "summary-mode",
          "Closeout completion summary remains read-only preview only",
          "Mode is read-only-preview-closeout-completion-summary."
        )
      : fail(
          "summary-mode",
          "Closeout completion summary remains read-only preview only",
    ...
    summary.status === "closeout-completion-summary-ready"
      ? pass(
          "summary-status",
          "Closeout completion summary is ready for planning only",
          "Status is closeout-completion-summary-ready."
        )
      : fail(
          "summary-status",
          "Closeout completion summary is ready for planning only",
    ...
    summary.upstreamArtifacts.closeoutFinalCheckpointStatus ===
      "closeout-final-checkpoint-ready"
      ? pass(
          "upstream-final-checkpoint",
          "Closeout final checkpoint status is valid",
          "Upstream closeout final checkpoint status is closeout-final-checkpoint-ready."
        )
      : fail(
          "upstream-final-checkpoint",
          "Closeout final checkpoint status is valid",
          `Unexpected upstream status: ${summary.upstreamArtifacts.closeoutFinalCheckpointStatus}`
        ),

    summary.completedChainSummary.some(
      (item) =>
        item.id === "day224-through-day243-summarized" &&
        item.status === "summarized"
    ) &&
    summary.completedChainSummary.some(
      (item) =>
        item.id === "closeout-final-checkpoint-confirmed" &&
        item.status === "confirmed"
    ) &&
    summary.completedChainSummary.some(
      (item) =>
        item.id === "planning-only-completion" &&
    ...
    summary.completedChainSummary.some(
      ...
      ? pass(
          ...
          "Day 224 through Day 243 chain is summarized for planning only",
          "Completion chain confirms planning-only completion and no live execution."
        )
      : fail(
          "completed-chain-summary",
          "Day 224 through Day 243 chain is summarized for planning only",
          "Completed chain summary is incomplete."
        ),
    ...
    summary.launchStatusSummary.some(
      (item) =>
        item.id === "execution-architecture-required" &&
    ...
    includesAll(summary.forbiddenExecutionSummary, requiredForbiddenActions)
      ? pass(
          ...
          "Summary includes every required forbidden execution class, including no launch authorization."
        )
      : fail(
          "forbidden-execution",
          "All forbidden execution classes remain blocked",
    ...
    includesAll(summary.completionBoundary.completedFor, [
    ...
    includesAll(summary.completionBoundary.notCompletedFor, [
      ...
      ? pass(
          ...
          "Completion is only for read-only planning and explicitly excludes public launch and live execution."
        )
      : fail(
          "completion-boundary",
          "Completion boundary is planning-only and explicitly not launch",
  ...
  const overallStatus: CloseoutCompletionValidatorStatus = checks.every(
  ...
  return {
    completionValidatorId:
      "controlled-paid-pilot-subscription-lock-boundary-closeout-completion-validator-v1",
    title:
      "NEXUS Controlled Paid Pilot Subscription Lock Boundary Closeout Completion Validator v1",
    mode: "read-only-preview-closeout-completion-validator",
    day: 244,
    overallStatus,
    validatedCompletionSummaryId: summary.completionSummaryId,
    checks,
    forbiddenExecutionValidation: requiredForbiddenActions,
    safetyConclusion:
      overallStatus === "pass"
        ? "Controlled paid pilot subscription lock boundary closeout completion summary validated as planning-only, launch-blocked, locked-by-default, owner-controlled, monetization-safe, audit-ready for review only, fallback-aware, rollback-aware, and non-executing."
        : "Controlled paid pilot subscription lock boundary closeout completion validation failed. Manual owner review required before continuation.",
    nextRecommendedStep:
      overallStatus === "pass"
        ? "Day 245: NEXUS Controlled Paid Pilot Subscription Lock Boundary Closeout Completion Checkpoint v1"
        : "Stop and manually review closeout completion validator failure before continuing.",
  };
}
