...
describe(
  "Riya limited internal pilot recommendation two execution",
  () => {
    it(
      "executes recommendation two of three and stops for mandatory owner review",
      async () => {
        const execution =
    ...
    it(
      "binds the exact Day 65 decision Day 64 preparation and complete source chain",
      async () => {
        const input =
          await day66ExecutionInput();
        ...
        expect(
          execution.sourceRecommendationReviewDecisionId,
        ).toBe(
    ...
    it(
      ...
      async () => {
        ...
        const rejectedInput =
          await day65DecisionInput({
            decisionId:
              "riya-owner-recommendation-two-execution-rejected-for-day-66",

            decision:
              "REJECT_AND_RETAIN_RECOMMENDATION_TWO_PREPARATION_ONLY",

            reason:
              "Owner rejected recommendation two execution and retained preparation only.",
          });
      ...
      async () => {
        ...
        const alternatePreparation =
          createRiyaLimitedInternalPilotRecommendationTwoPreparation(
            await day64PreparationInput({
              preparationId:
                "riya-limited-pilot-recommendation-two-preparation-alternate",
            }),
          );
      ...
      async () => {
        ...
        await expect(
          executeRiyaLimitedInternalPilotRecommendationTwo({
            ...
            executionId:
              "secret-riya-recommendation-two-execution",
          }),
        ).rejects.toThrow(
