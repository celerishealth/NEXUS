...
describe(
  "Riya limited internal pilot recommendation two preparation",
  () => {
    it(
      "prepares recommendation two without executing or generating it",
      async () => {
        const preparation =
          createRiyaLimitedInternalPilotRecommendationTwoPreparation(
    ...
    it(
      "binds the exact Day 63 review Day 62 execution owner decision preparation shadow tenant and owner",
      async () => {
        const input =
          await day64PreparationInput();
    ...
    it(
      "preserves clarification safety standards before recommendation generation",
      async () => {
        const preparation =
          createRiyaLimitedInternalPilotRecommendationTwoPreparation(
            await day64PreparationInput(),
    ...
    it(
      "rejects a Day 63 decision that did not approve recommendation two preparation",
      async () => {
        const rejectedReview =
          createRiyaOwnerLimitedInternalPilotRecommendationReviewDecision(
            await day63ReviewInput({
              decisionId:
                "riya-owner-recommendation-one-review-rejected-for-day-64",

              decision:
                "REJECT_AND_RETAIN_RECOMMENDATION_ONE_ONLY",

              reason:
                "Owner rejected continuation and retained recommendation one as the only pilot result.",
            }),
          );
        ...
        expect(() =>
          ...
          "approved Workforce Day 63",
        );
      },
    );

    it(
      "rejects tampered Day 63 evidence unsafe recomputed authority and preparation before owner review",
      async () => {
        const input =
          await day64PreparationInput();
        ...
        expect(() =>
          ...
          "cannot precede the recommendation one owner review decision",
        );
      },
    );
      ...
      async () => {
        ...
        expect(() =>
          createRiyaLimitedInternalPilotRecommendationTwoPreparation({
            ...
            preparationId:
              "secret-riya-recommendation-two-preparation",
          }),
        ).toThrow(
