...
function fixturePack() {
  ...
  return createMeeraQualificationFixturePack({
    fixturePackId:
...
function input() {
  return {
    ...
    executedAt:
      "2026-07-17T16:45:00.000Z",
  };
}

describe(
  "Meera qualification execution evidence",
  () => {
    it(
      "executes exactly twelve synthetic qualification fixtures",
      async () => {
        const ledger =
          await executeMeeraQualificationEvidence(
            input(),
          );
