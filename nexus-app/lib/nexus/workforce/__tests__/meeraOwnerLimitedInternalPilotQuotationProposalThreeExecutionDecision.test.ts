...
describe(
  "Meera owner limited internal pilot quotation/proposal three execution decision",
  () => {
    it(
      "approves quotation/proposal three execution without performing it",
      async () => {
        const decision =
          createMeeraOwnerLimitedInternalPilotQuotationProposalThreeExecutionDecision(
    ...
    it(
      "records rejection and retains quotation/proposal three preparation only",
      async () => {
        const decision =
          createMeeraOwnerLimitedInternalPilotQuotationProposalThreeExecutionDecision(
            await day103DecisionInput({
              decisionId:
                "meera-owner-recommendation-two-execution-rejected-day-103",

              decision:
                "REJECT_AND_RETAIN_QUOTATION_PROPOSAL_THREE_PREPARATION_ONLY",

              reason:
                "Owner rejected execution and retained quotation/proposal three at preparation only.",
            }),
          );
    ...
    it(
      "binds exact Day 102 preparation and complete source chain",
      async () => {
        const input =
          await day103DecisionInput();
    ...
    it(
      "preserves exact quotation/proposal three clarification scope",
      async () => {
        const decision =
          createMeeraOwnerLimitedInternalPilotQuotationProposalThreeExecutionDecision(
            await day103DecisionInput(),
      ...
      async () => {
        ...
        expect(() =>
          createMeeraOwnerLimitedInternalPilotQuotationProposalThreeExecutionDecision({
            ...
            decision:
              "INVALID_EXECUTION_DECISION" as Day103DecisionInput[
                "decision"
              ],
          }),
        ).toThrow(
