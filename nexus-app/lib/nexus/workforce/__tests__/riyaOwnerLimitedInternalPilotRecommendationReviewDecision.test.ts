...
describe(
  "Riya owner limited internal pilot recommendation review decision",
  () => {
    it(
      "approves recommendation two preparation only without authorizing recommendation two execution",
      async () => {
        const decision =
          createRiyaOwnerLimitedInternalPilotRecommendationReviewDecision(
    ...
    it(
      "records owner rejection and retains recommendation one as the only executed pilot recommendation",
      async () => {
        const decision =
          createRiyaOwnerLimitedInternalPilotRecommendationReviewDecision(
            await day63ReviewInput({
              decisionId:
                "riya-owner-recommendation-one-review-rejected-day-63",

              decision:
                "REJECT_AND_RETAIN_RECOMMENDATION_ONE_ONLY",

              reason:
                "Owner rejected continuation and retained the limited pilot at recommendation one only.",
            }),
          );
    ...
    it(
      "records exact recommendation one evidence and source bindings",
      async () => {
        const input =
          await day63ReviewInput();
    ...
    it(
      ...
      async () => {
        await expect(
          ...
          "Only the Riya limited-internal-pilot-bound owner",
        );
      },
    );
    ...
    it(
      "rejects a tampered Workforce Day 62 execution digest",
      async () => {
        const input =
          await day63ReviewInput();
