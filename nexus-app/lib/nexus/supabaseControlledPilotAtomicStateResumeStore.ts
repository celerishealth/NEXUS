...
export interface ControlledPilotAtomicStateResumeRecord {
  tokenId: string;
  tenantId: string;
  signalId: string;
  ownerId: string;
...
export interface ControlledPilotAtomicStateResumeStore {
  commitStateResume(
    record: ControlledPilotAtomicStateResumeRecord,
  ): Promise<ControlledPilotAtomicStateResumeResult>;
}

export interface SupabaseControlledPilotAtomicStateResumeStoreOptions {
  supabaseUrl: string;
...
function isRecordValid(
  ...
  return (
    ...
    normalizeInteger(record?.issuedAt) !== null &&
    normalizeInteger(record?.expiresAt) !== null &&
    normalizeInteger(record?.consumedAt) !== null &&
    normalizeInteger(record?.expectedStateVersion) !== null &&
...
export class SupabaseControlledPilotAtomicStateResumeStore
implements ControlledPilotAtomicStateResumeStore {
  ...
  constructor(
    options:
      SupabaseControlledPilotAtomicStateResumeStoreOptions,
  ) {
    const supabaseUrl =
    ...
    if (
      ...
      throw new Error(
        "Invalid server-only atomic state resume configuration.",
      );
    }
  ...
  async commitStateResume(
    ...
    try {
      ...
      if (
        ...
        row.status === "state-not-paused" ||
        row.status === "signal-state-mismatch"
      ) {
        return {
          status: row.status,
