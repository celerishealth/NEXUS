...
import {
  ...
  previewCustomerMemoryReviewQueueItem,
} from "../../../../lib/nexus/customerMemoryReviewQueueContract";
...
export async function GET() {
  const sample = {
    ...
    ownerReviewStatus: "pending-review" as const,
    createdAt: new Date().toISOString(),
  };

  return NextResponse.json({
    ok: true,
    service: "NEXUS Customer Memory Review Queue Contract",
    version: "v1",
    generatedAt: new Date().toISOString(),
    contract: customerMemoryReviewQueueContract,
    samplePreview: previewCustomerMemoryReviewQueueItem(sample),
  });
}
